 ...
 console.log(searchResults);
  ...
  useEffect(() => {
    if (searchResults.length > 0) {
      ...
      setFriendsList(searchResults.filter(u => u._id !== currentUser1._id).map(user => ({ ...user, isAdded: false })));
    }
  }, [searchResults]);
  ...
  const handleAddFriend = async (friend) => {
    try {
      ...
      setFriendsList(prev => prev.map(f =>
        f._id === friend._id ? { ...f, isAdded: true } : f
      ));
    } catch (error) {
              ...
              >
                ...
                <td className="py-6 px-6 text-right">
                  ...
                  >
                    ...
                    <span>{friend.isAdded ? 'Wait For Accept' : 'Add Friend'}</span>
                  </button>
                </td>
              </tr>
            ))}
